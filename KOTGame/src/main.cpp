...
int main() {
	...
	scene->CreateObject(CONTROL, "Player1", { 640.0f, 0 }, 1.0f, filename, 1.0f, 10.0f, 5.0f); //Create Player 1
	CheckTexture(filename);

	filename = "default2.png";
	...
	for (auto& obj : scene->objects) {
		...
		if (obj->tag == "Player1") {
			...
			//Body of player
			col = std::make_shared<Hurtbox>(BOXTYPE::Body, obj->size, obj->position + (obj->size * 0.5f));
		}
